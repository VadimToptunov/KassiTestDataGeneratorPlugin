...
object Checksums {
    ...
    // ---------------------------------------------------------------------
    // VIN (ISO 3779 / NHTSA 49 CFR 565) — 17 chars, check character at position 9.
    // ---------------------------------------------------------------------

    // Transliteration of letters to their numeric values; I, O, Q are not valid VIN characters.
    ...
    // ---------------------------------------------------------------------
    // ISBN-10 — mod-11 check character (ISBN-13 is EAN-13, see EuIdChecksums.ean13Check).
    // ---------------------------------------------------------------------
    ...
    // Letter values run A=10, B=12, … sequentially but skipping every multiple of 11 (11, 22, 33).
    ...
    /** ISO 6346 check digit for the first 10 chars (4 letters + 6 digits): Σ value·2^i mod 11, 10 → 0. */
    ...
    /** Validate a full 11-char ISO 6346 container code (4 letters + 6 digits + check digit). */
    ...
    // ---------------------------------------------------------------------
    // Spanish BBAN control digits — the two national check digits inside an ES IBAN (beyond mod-97).
    // Bank+branch (padded to 10 with a leading "00") and the 10-digit account each get one digit.
    // ---------------------------------------------------------------------
    ...
    /** Validate the two national control digits inside a Spanish IBAN (in addition to the mod-97 check). */
